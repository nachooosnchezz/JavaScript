...
// selecciono el input para añadir el texto
const añadirNombre = document.getElementById("añadirtexto");
// selecciono el botón para enviar el contenido
const enviarNombre = inputs[inputs.length - 3];
// selecciono el textarea
const nombreOrdenado = document.querySelector("textarea");
let nombres = [];

// configuramos el botón
enviarNombre.addEventListener("click", function(){
    nombreOrdenado.innerHTML = "";
    const nuevoNombre = añadirNombre.value;
    if (nuevoNombre === ""){
        alert("Por favor, ingresa un nombre.");
    }
    // añado el nombre escrito
    nombres.push(nuevoNombre);
    // ordeno alfabéticamente los nombres añadidos
    nombres.sort();
    // actualizo el textarea para que aparezcan los nombres ordenados
    nombreOrdenado.value = nombres.join("\n");
    // borro el input
    añadirNombre.value = "";
});